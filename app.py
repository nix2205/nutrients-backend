...
#         l1 = f"{d['food'].replace('_',' ').title()}  {d['conf']*100:.0f}%"
#         l2 = f"{nut['calories']} kcal | {nut['protein']}g P  {nut['carbs']}g C  {nut['fat']}g F"
#         ly = max(0, y1 + 8)
#         bw = max(len(l1), len(l2)) * 7 + 14
#         draw.rectangle([x1+4, ly, x1+bw, ly+48], fill=col)
#         draw.text((x1+10, ly+4),  l1, fill='white', font=font_big)
#         draw.text((x1+10, ly+26), l2, fill='white', font=font_small)
#         results.append({
#             'id':           i+1,
#             'food':         d['food'],
